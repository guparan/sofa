...
void SofaTutorials::createActions()
{
...
void SofaTutorials::createToolbar()
{
...
SofaTutorials::SofaTutorials():recentlyOpenedFilesManager(Utils::getSofaPathPrefix() + "/config/Modeler.ini")
    ,runSofaGUI(NULL)
{
...
void SofaTutorials::updateViewerList()
{
    //Clear the menu
...
void SofaTutorials::rebuildLibrary()
{
    library->clear();
...
void SofaTutorials::closeEvent( QCloseEvent *e)
{
...
void SofaTutorials::fileNew( Node* root)
{
...
void SofaTutorials::clearTab()
{
    if (graph)	fileNew();
}


void SofaTutorials::newTab()
{
...
void SofaTutorials::closeTab()
{
    if (sceneTab->count() == 0) return;
    closeTab(tabGraph);
}

bool SofaTutorials::closeTab(int i)
{
    if (i<0) return true;
    return closeTab(sceneTab->widget(i));
}

bool SofaTutorials::closeTab(QWidget *curTab, bool forceClose)
{
...
void SofaTutorials::fileOpen(const QUrl &u)
{
#ifdef WIN32
...
void SofaTutorials::fileOpen(std::string filename)
{
...
void SofaTutorials::fileRecentlyOpened(QAction* act)
{
    fileOpen(act->text());
}

void SofaTutorials::pressedGNodeButton()
{
    newGNode();
...
void SofaTutorials::exportSofaClasses()
{
...
void SofaTutorials::loadPreset(QAction* act)
{
...
void SofaTutorials::changeTabName(GraphModeler *graph, const QString &name, const QString &suffix)
{
    QWidget *tabGraph=0;
...
void SofaTutorials::resizeEvent(QResizeEvent * /*event*/)
{
...
void SofaTutorials::graphModifiedNotification(bool modified)
{
...
void SofaTutorials::changeInformation(QTreeWidgetItem *item, QTreeWidgetItem * /* previous */)
{
...
void SofaTutorials::componentDraggedReception( std::string description, std::string // categoryName
...
void SofaTutorials::changeComponent(const std::string &description)
{
    infoItem->setText(description.c_str());
}


void SofaTutorials::newGNode()
{
...
void SofaTutorials::changeCurrentScene( int id)
{
...
void SofaTutorials::changeCurrentScene(QAction* act)
{
    sceneTab->setCurrentWidget(mapWindow[act] );
}


void SofaTutorials::changeNameWindow(std::string filename)
{
...
void SofaTutorials::dropEvent(QDropEvent* event)
{
...
void SofaTutorials::editTutorial(const std::string& filename)
{
...
// void SofaTutorials::propertyDockMoved(Q3DockWindow::Place p)
// {
...
void SofaTutorials::openTutorial()
{
    if (tuto)
    {
    ...
    tuto=new SofaTutorialsManager(this, "tutorial");
...
void SofaTutorials::runInSofa()
{
    if (sceneTab->count() == 0) return;
    Node* root=graph->getRoot();
    runInSofa(graph->getFilename(), root);
}
void SofaTutorials::runInSofa(	const std::string &sceneFilename, Node* root)
{
    if (!root) return;
    // Init the scene
...
void SofaTutorials::redirectStdout()
{
...
void SofaTutorials::redirectStderr()
{
    QProcess* p = ((QProcess*) sender());
    if( !p )
    {
...
void SofaTutorials::sofaExited(int exitCode, QProcess::ExitStatus existStatus)
{
    QProcess *p = ((QProcess*) sender());
...
void SofaTutorials::removeTemporaryFiles(const std::string &f)
{
...
void SofaTutorials::dragEnterEvent( QDragEnterEvent* event)
{
    QString text;
    if (event->mimeData()->hasText())
        text = event->mimeData()->text();
...
void SofaTutorials::dragMoveEvent( QDragMoveEvent* event)
{
    QString text;
    if (event->mimeData()->hasText())
        text = event->mimeData()->text();
...
void SofaTutorials::searchText(const FilterQuery& query)
{
...
/*****************************************************************************************************************/
//runSofa Options
void SofaTutorials::changeSofaBinary()
{
...
void SofaTutorials::showPluginManager()
{
    plugin_dialog->show();
}

void SofaTutorials::displayMessage(const std::string &m)
{
    QString messageLaunch(m.c_str());
    statusBar()->showMessage(messageLaunch,5000);
}

void SofaTutorials::displayHelpModeler()
{
